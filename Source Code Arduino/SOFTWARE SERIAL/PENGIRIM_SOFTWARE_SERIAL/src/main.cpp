#include <Arduino.h>
#include <SoftwareSerial.h> // 1. Tambahkan library Software Serial

// Deklarasi Pin
const int buttonPin = 2;
const int ledPin = 8;

// 2. Buat jalur komunikasi baru (RX di Pin 10, TX di Pin 11)
...
void setup() {
  ...
  // Memulai komunikasi Serial Port untuk melihat counter di PC
  Serial.begin(9600);
  Serial.println("Sistem Siap. Silakan tekan tombol!");

  // 3. Memulai komunikasi Software Serial (Jalur ke Arduino 2)
  mySerial.begin(9600);
}
  ...
  if ((millis() - lastDebounceTime) > debounceDelay) {
    ...
    if (reading != buttonState) {
      ...
      if (buttonState == LOW) {
        ...
        // --- 4a. Tampilkan informasi di Serial Monitor (PC Arduino 1) ---
        Serial.print("Tombol ditekan! Total: ");
        Serial.print(pressCount);
        Serial.print(" | Status LED: ");
        Serial.println(ledState == HIGH ? "MENYALA" : "MATI");

        // --- 4b. Kirim informasi ke Arduino 2 via Software Serial ---
        mySerial.print("Info dari Arduino 1: Tombol ditekan ke-");
        mySerial.print(pressCount);
        mySerial.print(" | LED: ");
        mySerial.println(ledState == HIGH ? "NYALA" : "MATI");
      }
    }
  }
